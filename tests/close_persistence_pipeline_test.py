...
def test_publish_message_to_sns():
    ...
    message = {"BatchId": batchId, "Status": "Persistence process completed"}
...
def test_delete_sqs_message():
    with pytest.raises(Exception):
        ...
        response = sqs.create_queue(QueueName='dev-dot-sdc-waze-data-persistence-orchestration',
                         Attributes={'FifoQueue': "false", 'DelaySeconds': "60", 'MaximumMessageSize': "262144",
...
@mock_sqs
def test_push_batchid_to_nightly_sqs_queue_raises_exception():
    with pytest.raises(Exception):
        sqs = boto3.client('sqs', region_name='us-east-1')
        ...
        queue_events.append(queue_event)
        close_pipeline_obj = ClosePipeline()
        close_pipeline_obj.push_batchid_to_nightly_sqs_queue(queue_events, None)


@mock_sqs
def test_push_batchid_to_nightly_sqs_queue():
    sqs = boto3.client('sqs', region_name='us-east-1')
